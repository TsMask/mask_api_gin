...
// FindByPage 分页查询列表数据
func (r *SysConfig) FindByPage(query map[string]any) map[string]any {
	return r.sysConfigRepository.SelectByPage(query)
}

// FindById 通过ID查询信息
func (r *SysConfig) FindById(configId string) model.SysConfig {
	if configId == "" {
...
// DeleteByIds 批量删除信息
func (r *SysConfig) DeleteByIds(configIds []string) (int64, error) {
	// 检查是否存在
...
func (r *SysConfig) FindValueByKey(configKey string) string {
...
func (r *SysConfig) CheckUniqueByKey(configKey, configId string) bool {
...
// getCacheKey 组装缓存key
func (r *SysConfig) getCacheKey(configKey string) string {
	return constCacheKey.SysConfigKey + configKey
}

// CacheLoad 加载参数缓存数据 传入*查询全部
func (r *SysConfig) CacheLoad(configKey string) {
	// 查询全部参数
...
func (r *SysConfig) CacheClean(configKey string) bool {
	key := r.getCacheKey(configKey)
